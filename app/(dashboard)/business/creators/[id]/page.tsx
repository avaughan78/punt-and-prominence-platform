...
export default async function CreatorProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  type RawMatch = { id: string; status: string; created_at: string; offer: { title: string; value_gbp: number; fee_gbp: number | null; invite_type: string; category: string } | null }
  const matches = (creator.matches ?? []) as unknown as RawMatch[]
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      >
        {/* Header band — Instagram gradient if applicable */}
        <div
          className="h-24"
          style={{
